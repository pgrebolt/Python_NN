# Definim la funció que imprimeix el resultat actual
def imprimir_resultat(paraula_clau, ll_trobades, nintents):
	# Imprimim l'estat del joc
	for lletra in paraula_clau: #AIXÒ ES POT POSAR COM UNA FUNCIÓ
		if lletra in ll_trobades:
			print(lletra, end='')
		else:
			print("*", end='')
	print("\nIntents restants: ", 10-nintents)
	print()
...
while victoria == False:
	...
	if len(nova_lletra) > 1:
		...
		# Imprimim l'estat del joc
		imprimir_resultat(paraula, lletres_trobades, intents)
